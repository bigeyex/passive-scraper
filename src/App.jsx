...
function App() {
    ...
    const sendHtmlData = (label, data) => {
        if (wsRef.current && wsRef.current.readyState === WebSocket.OPEN) {
            ...
            addLog('success', `Sent HTML for "${label}" to Python script`);
        } else {
            addLog('warning', `WebSocket not connected. Cannot send HTML for "${label}"`);
        }
    };
